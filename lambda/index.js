    ...
    handle(handlerInput) {
        ...
        if (enfoqueNutricional === 'bajo en carbohidratos') {
            ...
            consejos = [
                ...
                "Bebe suficiente agua y limita el consumo de bebidas azucaradas o con cafeína.",
                "Trabaja con un dietista o un endocrinólogo profesional dedicado para personalizar tu plan de alimentación y monitoreo."
            ];
        }
...
const GlucemiaHandler = {
    canHandle(handlerInput) {
        return Alexa.getIntentName(handlerInput.requestEnvelope) === 'GlucemiaIntent';
    },
    handle(handlerInput) {
        const nivelGlucemia = handlerInput.requestEnvelope.request.intent.slots.nivelGlucemia.value;
        const nivelGlucemiaFloat = parseFloat(nivelGlucemia);

        console.log('Valor del slot nivelGlucemia:', nivelGlucemiaFloat); // Para depuración

        let speechText = '';

        if (isNaN(nivelGlucemiaFloat)) {
            speechText = 'Lo siento, no pude entender el valor de tu glucemia. Por favor, intenta decirme el número nuevamente.';
            return handlerInput.responseBuilder
                .speak(speechText)
                .reprompt()
                .getResponse();
        ...
        // Definir rangos de glucemia y las recomendaciones asociadas
        if (nivelGlucemiaFloat < 70) {
            speechText = 'Tu nivel de glucemia está bajo. Te recomiendo que consumas una pequeña porción de carbohidratos rápidos, como una fruta, jugo o una cucharada de azúcar disuelta en agua, y que revises nuevamente tu nivel de glucemia en 15 minutos.';
        } else if (nivelGlucemiaFloat >= 70 && nivelGlucemiaFloat <= 140) {
            speechText = 'Tu nivel de glucemia está en un rango normal. Mantén una alimentación equilibrada y sigue monitoreando tu glucemia regularmente.';
        } else if (nivelGlucemiaFloat > 140) {
            speechText = 'Tu nivel de glucemia es alto. Te sugiero que bebas agua y evites consumir alimentos ricos en carbohidratos hasta que tus niveles bajen. También podrías considerar realizar una pequeña caminata para ayudar a reducir el nivel de glucemia, y consulta a tu médico si persiste.';
        }
...
const HelpIntentHandler = {
    ...
    handle(handlerInput) {
        ...
        const helpMessages = [
            "GlucoGuru te proporciona la información y el apoyo que necesitas para vivir una vida saludable con diabetes. ¿Quieres probarlo? Simplemente di: 'Qué alimentos debo evitar si tengo: tu tipo de diabetes' y te daré una sugerencia personalizada.",
            "¿Necesitas ayuda? Pregúntame sobre alimentos recomendados o ejercicios para controlar la diabetes. Di: 'Dame un consejo nutricional'.",
            "Estoy aquí para ayudarte con tus consultas sobre la diabetes. Puedes decir: '¿Cómo puedo equilibrar mis comidas?' tu tipo de comida y te daré consejos útiles.",
            "¿No sabes qué preguntar? Intenta con: '¿Cómo puedo mejorar mi dieta si tengo diabetes?' o 'Dame un consejo para controlar la glucosa'.",
        ];
...
const CancelAndStopIntentHandler = {
    ...
    handle(handlerInput) {
        const speakOutput = 'Hasta luego, fue un placer!';
